...
export function EquipmentStatusChart({ data }: { data: { status: string; count: number }[] }) {
  const chartData = data.map((d) => ({ name: STATUS_LABELS_AR[d.status] ?? d.status, value: d.count, status: d.status }));
...
export function FaultPriorityChart({ data }: { data: { priority: string; count: number }[] }) {
  const labels: Record<string, string> = { low: 'منخفضة', medium: 'متوسطة', high: 'عالية', critical: 'حرجة' };
  const colors: Record<string, string> = { low: '#94a3b8', medium: '#2f82ff', high: '#eab308', critical: '#dc2626' };
