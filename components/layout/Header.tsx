...
export default function Header({
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      ...
      const combined: SearchResult[] = [
        ...
        ...(faults ?? []).map((f: any) => ({ type: 'fault' as const, id: f.id, title: f.fault_number, subtitle: f.description, href: `/faults` })),
        ...(spareParts ?? []).map((p: any) => ({ type: 'spare_part' as const, id: p.id, title: p.part_name, subtitle: p.part_number ?? '', href: `/spare-parts` })),
      ];
      setResults(combined);
      setSearching(false);
      setOpen(true);
  ...
  const icons = { building: Building2, equipment: Cpu, fault: AlertTriangle, spare_part: Package };
    ...
    <header className="flex items-center justify-between border-b border-gray-200 bg-white px-4 py-3 lg:px-6">
      <div ref={boxRef} className="relative w-full max-w-sm">
        ...
        <input
          ...
          placeholder="ابحث عن مبنى، معدة، رقم أصل، رقم تسلسلي..."
        ...
        />
        {searching && <Loader2 className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 animate-spin text-gray-400" />}
  ...
  لا توجد نتائج مطابقة لـ &quot;{query}&quot;
      ...
      <div className="flex items-center gap-3">
        <Link href="/notifications" className="relative rounded-lg p-2 text-gray-500 hover:bg-gray-50">
